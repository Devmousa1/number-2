...
trait houz_sms {
	public function URL(){
		return "http://api.haozhuma.com/sms/?api=++action++&token={$this->getMyKey()}&";
	}
	...
	public function getNumber($app,$country,$opreator){
		...
		$action      = __FUNCTION__;
		$url             =  str_replace('++action++','getPhone',$this->URL())."sid={$app}";
		...
		if(preg_match("/}/",$result)){
			...
			if ($bought->phone != null){
				$result      = [true,['id'=>$bought->phone, 'phone'=>'+86'.$bought->phone]];
			} else {
				$result      = [false,'NoNo'];
			}
		}
	...
	public function getCode($id){
		$url              =  str_replace('++action++','getMessage',$this->URL())."phone={$id}&sid=22828";
		#$headers    = $this->get_headers();
		#$result        = $this->sendCurl($url,'GET',true,$headers);
		$result      = file_get_contents($url);
	...
	public function cancelNumber($id){
		...
		$url              =  str_replace('++action++','addBlacklist',$this->URL())."phone={$id}&country_code=cn&sid=22828";
		#$headers    = $this->get_headers();
		#$result        = $this->sendCurl($url,'GET',true,$headers);
